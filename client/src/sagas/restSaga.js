...
function* itemUpdateThread(chan) {
    let chnum = ch++;
    console.log('thread created ',chnum);
    try {
        const rest = yield getContext('restClient');
        while (true) {
            const id = yield take(chan);
            console.log('check '+chnum+' '+id);
            const item = yield call([rest, rest.apis.all.getItem], {
                id
            });
...
function* checkRequestStatusTask() {
    ...
    try {
        ...
        for (let i = 0; i < 3; i++) {
            threads.push(yield fork(itemUpdateThread, chan));
        }
